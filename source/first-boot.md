...
- 主板输入范围为 DC `12V ~ 24V`（3S-6S），供电电流建议不低于 `600 mA`；配套适配器为 `12V`，不表示主板只支持 12V。
- 散热器、风扇和风道无遮挡；风扇会在通电时启动，待系统启动完成会停止，后续风扇温控会在 CPU 温度 `> 55°C` 时启动，在温度降到`< 50°C` 时停止。
- 如使用 UART1/UART7，只连接匹配的 `3.3V`电平的 TX/RX 和 GND共地，通用 USB-UART 适配器默认不接 VCC。
...
在断电状态下完成相机、供电、网络和可选 UART 调试线连接，然后使用配套的 `12V` 适配器给设备上电等待系统启动；如使用其他输入电源，必须满足硬件页公布的输入范围和供电边界，系统上电时风扇会启动，靠外边的绿色 LED 灯会常亮（首批板是绿色，后续板为了更好区分改成了红色led），等到系统启动完成风扇会停止，靠里面的绿色 LED 会闪烁。启动过程中不要插拔相机/FPC/同轴线，不要停止或重配 `cam-service`，也不要同时启动多个占用相机资源的应用。

## 3. 网络和登录

首次登录建议使用有线以太网。需要把板载 Wi-Fi 配置成 AP 热点或连接路由器时，见 [Wi-Fi 配置](wifi-configuration.md)；切换 Wi-Fi 模式会重置 `wlan0`，不要依赖同一条 Wi-Fi SSH 会话完成切换。
...
```bash
ETHERNET_IFACE=enp1s0  # 改成实际开发机以太网接口
ip link
