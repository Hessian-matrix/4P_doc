...
```{note}
本页用于处理部分前期系统在插入 TF 卡后系统无法识别的问题。该问题属于板端 boot 分区中的设备树配置问题，不能通过重新挂载、格式化 TF 卡或重新部署 `/root/demo` 运行包解决。20260818后出厂的系统已经修复了此问题，如果已经可以正常识别到TF卡，就不需要看这部分内容。
```
...
下面直到 `reboot` 之前的命令全部直接在这个 X5 SSH 终端中执行，不要在命令外再嵌套 SSH。前文示例使用出厂默认地址；地址已修改时按实际值连接。
