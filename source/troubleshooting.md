...
| 常见原因 | 客户端软件解码或渲染吞吐不足；四路 `1280x1088@60fps` 对客户端压力较高。 |
| 恢复/需收集信息 | 更换支持 H.265 硬解的播放器，或降低 fps/播放路数；收集客户端型号、播放器、codec 和帧率。 |
...
| 检查 | `ls -l /dev/ttyS1 /dev/ttyS7`；按产品确认后的 pinout、方向和共地要求核对；确认前禁止接 USB-UART VCC、5V TTL 或 RS-232。 |
| 正常结果 | 设备节点存在，双方 8N1/raw/no-flow-control 参数一致。 |
| 常见原因 | 端口选错、波特率不一致、TX/RX 线序错误、对端未发送。 |
| 恢复/需收集信息 | 收集端口、baud、mode、产品已确认的接线说明；确认前不要接入 USB-UART VCC、5V TTL 或 RS-232。 |
