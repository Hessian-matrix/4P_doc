...
头文件：`include/sc132camera.h`。ABI 仍为 2.0（`SC132_ABI_VERSION_MAJOR=2`、`SC132_ABI_VERSION_MINOR=0`）；结构布局、导出符号、SONAME 和 `libsc132.so.2.1.0` 文件名保持不变。`SC132_FRAME_TIMESTAMP_SEMANTICS_VERSION=2` 明确表示 `software_gpio` 下逐路 `timestamp_ns` 已由触发时刻升级为触发参考下的曝光中值；依赖旧语义的 consumer 必须同步升级。
状态码：
...
- `sample_rate_hz`只接受`25/50/100/200/500/1000/2000`；当前只接受 sensor timestamp FIFO 读取模式。
