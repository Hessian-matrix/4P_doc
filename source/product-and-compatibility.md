...
| 交付物 | 当前版本 | 用户入口 |
|---|---|---|
| 文档 | `1.1.0` | 本站与 `4P_doc` 仓库。 |
| non-ROS demo/运行包 | `1.1.0` | `/root/demo`，来自 `RoboBaton_4p_demo` 的 `demo/`。 |
| ROS2 package/install | `1.1.0` | `/root/ros2_demo/install`，包名 `robobaton_4p_ros2_demo`。 |
...
| 相机配置 | 默认`1280x1088@30fps`；仅支持`25fps`和`30fps`。 |
...
```{note}
出厂系统镜像版本标识、兼容范围和只读查询命令将在产品版本方法定版后补充。本文档、non-ROS demo和ROS2 package的`v1.1.0`不单独代表出厂系统镜像版本。
```
...
部署时按整包更新和校验：non-ROS使用`manifest.sha256`，ROS2 install使用archive checksum和runtime `abi_manifest.sha256`。不要只替换单个可执行文件或单个`.so`；文档、运行包和ROS2 package的`1.1.0`应作为同一v1.1.0发布集合使用。
