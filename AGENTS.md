...
├── image/             # 中英共享的图片资产
├── ch/                # 中文文档（Sphinx srcdir，conf.py language = "zh_CN"）
...
5. ch/（中文）与 en/（英文）公开内容变更时必须保持技术信息一致、页面一一对应；新增页面必须同时更新两棵树的 Sphinx 目录和链接检查。
6. 修改后至少对 ch/ 与 en/ 两棵树运行 Sphinx 严格 HTML 构建和 linkcheck，并运行 `git diff --check`；在主仓运行 `python3 tests/repository_policy_test.py`。
